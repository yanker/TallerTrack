...
export const getUpcomingMaintenance = query({
  ...
  handler: async (ctx) => {
    ...
    for (const item of scheduled) {
      ...
      if (item.everyXYears) {
        const lastMaintenanceDate = latestRecord ? new Date(latestRecord.repairDate) : vehiclePurchaseDate;

        const yearsSinceLastMaintenance = (currentDate.getTime() - lastMaintenanceDate.getTime()) / (1000 * 60 * 60 * 24 * 365.25);
      ...
      if (item.everyXKm) {
        ...
        if (currentKm >= nextMaintenanceKm) {
          isUpcoming = true;
          nextDueInfo += (nextDueInfo ? ' | ' : '') + `Vencido por ${currentKm - nextMaintenanceKm} km`;
        } else {
          const kmRemaining = nextMaintenanceKm - currentKm;
          if (kmRemaining <= item.everyXKm * 0.1) {
            isUpcoming = true;
            nextDueInfo += (nextDueInfo ? ' | ' : '') + `Próximo en ${kmRemaining} km`;
          }
        }
      }
...
export const exportScheduledMaintenance = query({
  args: {},
  handler: async (ctx) => {
    const userId = await requireAuth(ctx);
    ...
    const recordsWithVehicles = await Promise.all(
      records.map(async (record) => {
        ...
        return {
          ...
          cada_x_years: record.everyXYears || '',
          cada_x_km: record.everyXKm || '',
          observaciones: record.observations.replace(/<[^>]*>/g, ''), // Strip HTML
          estado: record.isActive ? 'Activo' : 'Inactivo',
        };
      })
    );

    return recordsWithVehicles.sort((a, b) => a.marca.localeCompare(b.marca));
  },
});
