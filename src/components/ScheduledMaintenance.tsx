      ...
      {upcomingMaintenance.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <h3 className="text-lg font-semibold text-red-800 mb-3">
            ⚠️ Mantenimientos Próximos o Vencidos ({upcomingMaintenance.length})
          </h3>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <h3 className="text-lg font-semibold mb-4">
              {editingScheduled ? "Editar Mantenimiento Programado" : "Nuevo Mantenimiento Programado"}
            </h3>
              ...
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="mt-2 space-y-1">
                    {scheduled.everyXYears && (
                      <p className="text-sm text-blue-600">
                        📅 Cada {scheduled.everyXYears} años
                      </p>
                    )}
                    {scheduled.everyXKm && (
                      <p className="text-sm text-blue-600">
                        🛣️ Cada {scheduled.everyXKm.toLocaleString()} km
                      </p>
                    )}
                    <p className="text-sm text-gray-700">{scheduled.observations}</p>
                    <div className="flex items-center gap-2">
                      <span className={`inline-block w-2 h-2 rounded-full ${
                        scheduled.isActive ? 'bg-green-500' : 'bg-gray-400'
                      }`}></span>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() => handleDelete(scheduled._id)}
